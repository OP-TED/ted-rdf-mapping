...
The methodology that we describe here, focuses on a few steps and artefacts that are at the core of the diagram depicted in <<mapping-creation>>. This diagram shows only a part of the link:mapping-lifecycle.adoc#mapping-lifecycle[mapping lifecycle].
...
#### Write RML rules (or convert YARRRML rules to RML)
If in the previous step we have defined the mapping rules in YARRRML, then this step consists of the simple action of executing link:cli-toolchain.adoc[the tool that generates RML out of YARRRML].
...
1. `rml:referenceFormulation` : defining the parser for the file. In the case of the XML notices we are using XPath, while for the CSV/JSON files we are using `ql:CSV/ql:JsonPath`
1. `rml:iterator` : the path were the RML mapping starts iterating for this Organisation mappings
...
### Document technical and philosophical issues and create tasks to find solutions for them
