...
image::maping_development_lifecycle.png[width=100%]
...
The notice classification scheme is usually subject to regulations and described in multiple forms starting from PDF documents, XSD schemes, and other documents. All possible documentation and modelling artefacts are **analysed in order to produce a conceptual mapping** (see xref:conceptual-mapping-structure.adoc[conceptual mapping structure] ). This artefact, stored as a workbook, establishes the correspondence of the TED notice fields, to XPath expressions and to EPO structures.
...
To put the SPARQL assertions into practice, the sample data, initially selected *sample data is automatically transformed into RDF*, using the RML mapping rules. And on each output, the *SPARQL queries are applied* resulting in a report, which indicates which query yielded a positive and which a negative result. The resulting validation reports can be checkecd to detect mistakes in the RML transformation rules or teh conceptual mapping.
