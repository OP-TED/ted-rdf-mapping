...
The notice classification scheme is usually subject to regulations and described in multiple forms starting from PDF documents, XSD schemes, and other documents. All possible documentation and modelling artefacts are **analysed in order to produce a conceptual mapping** (see link:conceptual-mapping-structure.adoc[conceptual mapping structure] ). This artefact, stored as a workbook, establishes the correspondence of the TED notice fields, to XPath expressions and to EPO structures.
