...
The methodology that we describe here, focuses in a few steps and artefacts that are at the core of the diagram depicted in <<mapping-creation>>. This diagram shows only a part of the xref:partial$methodology/mapping-lifecycle.adoc[mapping lifecycle].
...
When we take a look at the way how the RML rules are writen (see xref:technical-mapping-modularisation.adoc[technical mapping chapter]) we notice that it is a set of TripleMap. Each one contains:
...
Test data (XML notices) are the main input of our process pipline. To now how he are preparing them please se the xref:preparing-test-data.adoc[dedicated chapter].
