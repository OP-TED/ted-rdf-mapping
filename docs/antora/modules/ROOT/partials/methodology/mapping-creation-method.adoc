...
The methodology that we describe here, focuses on a few steps and artefacts that are at the core of the diagram depicted in <<mapping-creation>>. This diagram shows only a part of the xref:mapping-lifecycle.adoc#mapping-lifecycle[mapping lifecycle].
